...
```go
result, err := channels.Search(ctx, shopware.NewCriteria(), shopware.ApiContext{})
// result.Total           -> int
...
ids, err := channels.SearchIDs(ctx, shopware.NewCriteria(), shopware.ApiContext{})
```
...
## DAL context (language, version, …)

`ApiContext` carries the DAL context headers. The zero value sends none. Unset
fields are omitted.

```go
inherit := true
apiCtx := shopware.ApiContext{
	LanguageID:       shopware.DefaultSystemLanguageID,
	VersionID:        shopware.DefaultLiveVersionID,
	Inheritance:      &inherit,
	SkipTriggerFlows: nil, // omitted
}

result, err := products.Search(ctx, shopware.NewCriteria(), apiCtx)
```
